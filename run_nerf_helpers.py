...
def piecewise_constant_pdf(bins, weights, N_samples, use_stratified_sampling= True, pytest=False):
    # Get pdf
...
""" 
def piecewise_constant_pdf(bins, weights, N_samples, 
			  use_stratified_sampling, pytest=False):
    eps = 1e-5 
    #Get Pdf
    weights += eps
    pdf = weights / torch.sum(weights, -1, keepdim=True)
    ...
    u = u.contiguous()
    inds = torch.searchsorted(cdf, u, right=True)
    x0 = torch.max(torch.zeros_like(inds-1), inds-1)
    x1 = torch.min((cdf.shape[-1]-1) * torch.ones_like(inds), inds)
    inds_g = torch.stack([x0, x1], -1)  # (batch, N_samples, 2)
